<?php
/**
 * Course data.
 * Images: Lakshmi's artwork from assets/images/artwork/.
 */

return [
    [
        'id' => 'intro-acrylic',
        'name' => 'Introduction to Acrylic Painting',
        'level' => 'Beginner',
        'duration' => '4 sessions, 2 hours each',
        'format' => 'In-person, Celbridge — dates via WhatsApp',
        'description' => 'A gentle introduction to acrylics — colour mixing, brush control, and building a simple composition from start to finish. Mistakes are welcome here.',
        'image' => 'images/artwork/number-2477.jpeg',
        'image_alt' => 'Art training with Lakshmi Sridhar',
    ],
    [
        'id' => 'watercolour-basics',
        'name' => 'Watercolour Basics',
        'level' => 'Beginner',
        'duration' => '3 sessions, 2 hours each',
        'format' => 'In-person, Celbridge — dates via WhatsApp',
        'description' => 'Learn washes, wet-on-wet, and soft layering. Ideal if you love luminous colour and want to find your own rhythm with water and pigment.',
        'image' => 'images/artwork/german-shepherd.jpeg',
        'image_alt' => 'Art training with Lakshmi Sridhar',
    ],
    [
        'id' => 'advanced-techniques',
        'name' => 'Advanced Techniques',
        'level' => 'Intermediate',
        'duration' => '6 sessions, 2.5 hours each',
        'format' => 'In-person, Celbridge — dates via WhatsApp',
        'description' => 'For artists ready to deepen their practice — texture, layering, composition, and finishing work with more confidence and intention.',
        'image' => 'images/artwork/bowl-of-limes.jpeg',
        'image_alt' => 'Art training with Lakshmi Sridhar',
    ],
    [
        'id' => 'kids-workshops',
        'name' => "Kids' Art Workshops",
        'level' => 'Beginner',
        'duration' => 'Single session or a short series',
        'format' => 'In-person, Celbridge',
        'description' => 'Playful, encouraging sessions for young artists — colour exploration, simple techniques, and the joy of making something with their own hands.',
        'image' => 'images/artwork/on-the-rocks.jpeg',
        'image_alt' => 'Art training with Lakshmi Sridhar',
    ],
];
